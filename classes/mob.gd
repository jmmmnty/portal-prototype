...
func apply_camera_rotation( new_rotation : Transform2D )->void:
	...
	Global.view_rotation = view_rotation
	if get_node_or_null ( "Camera2D" ):
		$Camera2D.transform = view_rotation
		$Camera2D.overlay_camera.transform = view_rotation
...
func set_copy_position() -> void:
	$RemoteVisuals.transform = near_portal.relative_tranform_to_other_side( transform )
	var rotated_transform : Transform2D = Transform2D(actual_rotation, Vector2.ZERO )
...
func clip_collision( portal : portal_plane, offset : Vector2 ) -> void:
	# TODO this causes crash sometimes maybed
...
func clip_copy_collision() -> void:
	$RemoteCollision.transform = near_portal.transform_to_other_side( transform ).translated_local( -position )
