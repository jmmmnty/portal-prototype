...
func add_wall( wall : solid_wall ) -> void:
	...
	occluder.set_base_shape( wall.global_transform * wall.shadow_poly )
...
func slice_occluders( portals : Array[portal_plane], player : Mob ) -> void:
	...
	view_rect.position = player.global_position  - view_rect.size / 2
	...
	for occluder : VisionOccluder in $vision.get_children():
		...
		for portal in visible_portals:
			...
			portal_rect.position = portal.other_side.subview_camera.global_position
			var translator : Transform2D = portal.transform_to_other_side( portal.global_transform )
		...
		if occluder.base_rect.intersects( view_rect ):
			...
			for portal in visible_portals:
				var shadow : PackedVector2Array = portal.global_transform * portal.occlusion_polygon
