...
## Camera that sees area around this portal
...
func initialize() -> void:
	...
	# SubViewport sees area around portal
	subview = base_subviewport.new()
	subview.world_2d = get_parent().get_viewport().world_2d
	...
	subview.canvas_item_default_texture_repeat = Viewport.DEFAULT_CANVAS_ITEM_TEXTURE_REPEAT_ENABLED
	add_child( subview )
	
	# camera for the above viewport
	subview_camera = Camera2D.new()
	subview_camera.position = global_position
...
func calc_tele_transform() -> Transform2D:
	...
	ret.origin =  other_side.global_position - global_position
	return ret
...
func connect_signals()->void:
	...
	projection.position = global_position
...
func update_position( viewer_position : Vector2 )->void:
	...
	other_side.subview_camera.position = other_side.global_position + floor(offset)
	other_side.subview.size = ceil(view_size)
	projection.texture_offset = -floor(offset)
	return


## Takes pos in global coordinates
func portal_side( pos : Vector2 )->int:
	# -1 is on "front" side
	# +1 is on "back" side
	# 0 is on top of portal
	var r : Vector2 = pos - global_position
...
## Tells which side of the portal a point is
## Returns -1, 0, +1
## -1 is "front" side
## +1 is "back" side
## 0 is on top of portal
func portal_side_local( point : Vector2 )->int:
	var tmp : float= point.dot( normal )
	# rounding errors
	if abs(tmp) < 0.005:
		return 0
	return sign( tmp )

## Takes point in global coordinates
func distance_from_plane( point : Vector2 ) -> float:
	var closest_point : Vector2 = Geometry2D.get_closest_point_to_segment( point, global_position, global_position + Vector2( point_right ) )
...
func point_to_other_side( point : Vector2 ) -> Vector2:
	return other_side.global_transform * ( point * global_transform * rotation_transform.inverse() )


## Takes rotation transform
...
## matrix must be in global coordinates
func transform_to_other_side( matrix : Transform2D ) -> Transform2D:
	# TODO make work when portals are under different nodes
	return other_side.global_transform * rotation_transform * matrix * global_transform.inverse()


func polygon_to_other_side( polygon : PackedVector2Array ) -> PackedVector2Array:
	return other_side.global_transform * ( polygon * global_transform * rotation_transform.inverse() )


## Returns true if point is in between the portal endpoints.
## "point" is position in global coordinates
...
func point_is_in_portal( point : Vector2 ) -> bool:
	...
	var local : Vector2 = global_position - point
